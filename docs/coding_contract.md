...
utils/numeric.py 集中定义相对及时间/Compute/VRAM/功率/费用绝对容差 1e-9。A8 全窗口 gate 使用独立 power tolerance。EPS_NORM/EPS_RATIO=1e-9 为数值保护，EPS_DIRECTION=1e-6 保护零权重。身份包含 MS、OS、按时间容差量化的 starts。
