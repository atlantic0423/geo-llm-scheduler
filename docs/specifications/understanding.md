...
当前对象为静态、离线、确定性的 P→D；P/D 同 Region，跨实例增加 KV delay；双资源并发；秒/kW/kWh/USD；半开区间；两状态功率；batch 后功率为零；固定 900 秒尾窗。旧 10-action 和小时实现约定不覆盖当前 8-action/Coding Contract。
