...
正式公平性优先等wall-clock：用足够大generations和相同seconds，记录elapsed与未完成代；初始化/SSGS/exact/coalition/Polish都计时。当前子问题边界软停止，不能声称严格等CPU时间。固定代数用于回放与计数控制。

summary含seed、实例/配置/源码hash、Python、耗时、exact/feasible/rebuild、Archive尝试/贡献、Trigger触发/成功率、算子统计。trace每步保存state/action/budget/effective/attempts/reward/scalar/目标增量/计时，每个子代保存Archive目标集及elapsed用于anytime曲线。qtable保存值/访问/选择/更新数。
